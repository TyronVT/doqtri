...
exports.X402Server = exports.CDP_SERVER_DEVELOPMENT_NETWORKS = exports.CDP_SERVER_DEVELOPMENT_SVM_NETWORKS = exports.CDP_SERVER_DEVELOPMENT_EVM_NETWORKS = exports.CDP_SERVER_DEFAULT_NETWORKS = exports.CDP_SERVER_DEFAULT_SVM_NETWORKS = exports.CDP_SERVER_DEFAULT_EVM_NETWORKS = void 0;
...
const server_extensions_js_1 = require("./server-extensions.js");
const smart_account_js_1 = require("./smart-account.js");
 ...
 */
exports.CDP_SERVER_DEFAULT_EVM_NETWORKS = [constants_js_1.baseMainnetCaip2];
/**
 * Default Solana networks (production) — Solana mainnet.
 */
exports.CDP_SERVER_DEFAULT_SVM_NETWORKS = [constants_js_1.solanaMainnetCaip2];
/**
 * Default networks for production: Base mainnet + Solana mainnet.
 */
exports.CDP_SERVER_DEFAULT_NETWORKS = [
    ...exports.CDP_SERVER_DEFAULT_EVM_NETWORKS,
    ...exports.CDP_SERVER_DEFAULT_SVM_NETWORKS,
];
/**
 * Default EVM networks for development — Base Sepolia (`eip155:84532`).
 */
exports.CDP_SERVER_DEVELOPMENT_EVM_NETWORKS = [constants_js_1.baseSepoliaCaip2];
/**
 * Default Solana networks for development — Solana Devnet.
 */
exports.CDP_SERVER_DEVELOPMENT_SVM_NETWORKS = [constants_js_1.solanaDevnetCaip2];
/**
 * Default networks for development: Base Sepolia + Solana Devnet.
 */
exports.CDP_SERVER_DEVELOPMENT_NETWORKS = [
    ...exports.CDP_SERVER_DEVELOPMENT_EVM_NETWORKS,
    ...exports.CDP_SERVER_DEVELOPMENT_SVM_NETWORKS,
];
/**
 ...
 */
function resolveServerCredentials(config) {
    const rawEnv = config.environment ??
        process.env.CDP_X402_SERVER_ENVIRONMENT;
    const environment = rawEnv === "development" ? "development" : "production";
    return {
 ...
 */
async function provisionServerAccounts(credentials, payToConfig, need) {
    const cdpClient = new cdp_js_1.CdpClient({
    ...
    if (payToConfig.type === "smart") {
        ...
        try {
            ...
            if ((0, smart_account_js_1.isOwnerAlreadyHasSmartWalletError)(error)) {
                const existingAddress = await (0, smart_account_js_1.findSmartAccountByOwner)(cdpClient, ownerAccount.address);
                if (!existingAddress)
                    throw error;
        ...
        return {
            evmAddress: smartAccount.address,
            svmAddress,
 ...
 */
function isEvmOnlyScheme(scheme) {
 ...
 */
function getDefaultNetworksForScheme(scheme, environment) {
 ...
 */
function requiredNetworkFamilies(routes, environment) {
    const result = { evm: false, svm: false };
 ...
 * @param network - CAIP-2 network identifier (e.g. `"eip155:8453"`).
 */
function assertSchemeSupportsNetwork(scheme, network) {
    if (isEvmOnlyScheme(scheme) && !network.startsWith("eip155:")) {
        throw new Error(`Scheme "${scheme}" only supports EVM (eip155:*) networks. ` +
 ...
 */
function assertNonEmptyPayTo(payTo, network) {
    if (payTo === "") {
        const family = network.startsWith("eip155:")
            ...
            : network.startsWith("solana:")
                ? "Solana (`solana`)"
                : `"${network}"`;
        throw new Error(`No receiver address for ${family} network "${network}". ` +
 ...
 */
function fillX402RoutePayTo(route, evmAddress, svmAddress) {
    const accepts = Array.isArray(route.accepts) ? route.accepts : [route.accepts];
    const filled = accepts.map(option => {
        const network = option.network;
        assertSchemeSupportsNetwork(option.scheme, network);
        ...
        throw new Error(`Cannot fill vacant payTo for network "${network}": unrecognised network family. ` +
 ...
 */
function convertCdpRoute(route, evmAddress, svmAddress, environment, available) {
    const scheme = route.scheme ?? "exact";
    ...
    const networks = usingDefaultNetworks
        ? defaultNetworks.filter(network => {
            ...
            if (family === "evm")
                return available.evm;
            if (family === "svm")
                return available.svm;
            return true;
        })
        : route.networks;
    if (networks.length === 0) {
        throw new Error(`No receiver address available for any default network of scheme "${scheme}". ` +
            `Provide a receiver address via payToConfig (e.g. { type: "address", evm: "0x...", solana: "..." }), ` +
    ...
    const accepts = networks.map(network => {
        ...
        const payTo = network.startsWith("eip155:")
            ...
            : network.startsWith("solana:")
                ? svmAddress
                : (() => {
                    throw new Error(`Cannot resolve payTo for network "${network}": unrecognised network family.`);
                })();
 ...
 */
function parseRouteKeyForBazaar(pattern) {
 ...
 */
function routeHasEvmAccept(route) {
    const accepts = Array.isArray(route.accepts) ? route.accepts : [route.accepts];
    return accepts.some(option => option.network.startsWith("eip155:"));
}
/**
 ...
 */
function withAutoInjectedExtensions(pattern, route) {
    const bazaar = parseRouteKeyForBazaar(pattern);
    return {
        ...route,
        extensions: {
            ...(routeHasEvmAccept(route) && server_extensions_js_1.CDP_SUPPORTED_EXTENSIONS),
            ...(bazaar && { [server_extensions_js_1.CDP_EXTENSION_BAZAAR]: (0, server_extensions_js_1.buildBazaarDeclaration)(bazaar.method, bazaar.path) }),
            ...route.extensions,
        },
    };
}
 ...
 */
function resolveRoutes(routes, evmAddress, svmAddress, environment) {
    const result = {};
    const available = { evm: evmAddress !== "", svm: svmAddress !== "" };
    for (const [pattern, route] of Object.entries(routes)) {
        const resolved = "accepts" in route
 ...
 */
class X402Server extends server_1.x402HTTPResourceServer {
    /**
     ...
     * @internal
     */
    constructor(resourceServer, routes, payToEvmAddress, payToSvmAddress, ownerWallet) {
    ...
    static async create(config) {
        ...
        const facilitatorClient = (0, facilitator_js_1.createCdpFacilitatorClient)({
            apiKeyId: credentials.apiKeyId,
            apiKeySecret: credentials.apiKeySecret,
        });
        const resourceServer = new server_1.x402ResourceServer(facilitatorClient);
        for (const scheme of (0, server_extensions_js_1.getCdpDefaultSchemes)()) {
            resourceServer.register(scheme.network, scheme.server);
        }
        for (const ext of (0, server_extensions_js_1.getCdpExtensionRegistrations)()) {
         ...
         */
        const batchPayTos = new Set();
        for (const route of Object.values(resolvedRoutes)) {
            const accepts = Array.isArray(route.accepts) ? route.accepts : [route.accepts];
            for (const opt of accepts) {
                if (opt.scheme === "batch-settlement" && opt.payTo) {
                    batchPayTos.add(opt.payTo.toLowerCase());
                }
            }
        }
        if (batchPayTos.size > 1) {
            throw new Error(`batch-settlement routes must all share the same EVM receiver address within one ` +
        ...
        for (const route of Object.values(resolvedRoutes)) {
            ...
            for (const opt of accepts) {
                if (opt.scheme === "batch-settlement" && opt.payTo) {
                    batchAddresses.add(opt.payTo);
                }
            }
        }
        for (const addr of batchAddresses) {
            const reg = (0, server_extensions_js_1.getCdpBatchSettlementScheme)(addr);
            resourceServer.register(reg.network, reg.server);
        }
        // 9. Construct and initialize — syncs supported schemes with the facilitator.
        const instance = new X402Server(resourceServer, resolvedRoutes, evmAddress === "" ? undefined : evmAddress, svmAddress === "" ? undefined : svmAddress, ownerWallet);
